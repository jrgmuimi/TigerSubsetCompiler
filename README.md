...
The input files for several interactable programs are provided in the "inputs" folder. If you would like to see the MIPS assembly output or .S file for a program, then check the asm_output folder whenever you run the shell script on the target program. Beware that this folder gets cleared when calling "make clean".
